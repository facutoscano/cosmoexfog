...
def run(args, config):    
    ...
    ell_min, ell_max, delta_ell = cfg['ell_min'], cfg['ell_max'], cfg['delta_ell']
    ell_edges = np.arange(ell_min, ell_max + delta_ell + 1, delta_ell) 
    ...
    for mask_suffix in cfg.get('mask_variants', ['']):
        ...
        if sampler == 'iminuit':
            ...
            sim_indices = cfg.get('sim_indices')
            if sim_indices is None:
                sim_indices = range(cl_matrix.shape[0])
                
            for sim_idx in sim_indices:
                data_cl = cl_matrix[sim_idx]
                sim_label = "data" if sim_idx == 0 else f"sim{sim_idx}"
                
                for cut_info in cuts:
                    out_prefix = os.path.join(out_dir, f"mcmc_{sim_label}_{cut_info['name']}")
                    run_mcmc(cut_info, data_cl, cl_std, ell_edges, cfg['planck_pr3'], out_prefix, config, rank=0)
    ...
    print("\n======================================================")
    print("PIPELINE OF MULTIPOLES CUTTING FINISHED.")
    print("======================================================")